...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (openBrackets.includes(curSymbol)) {
      ...
      if (bracketsPair[curSymbol] === topElement) {
        stack.pop();
      } else {
        return false;
      }
...
function getCommonDirectoryPath(pathes) {
  const result = [];
  for (let i = 0; i < Math.min(...pathes.map((n) => n.length)); i += 1) {
    if (pathes.every((n) => n[i] === pathes[0][i])) result.push(pathes[0][i]);
    else break;
  }
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    if (position[0][i] === position[1][i] && position[1][i] === position[2][i] && (position[0][i] === 'X' || position[0][i] === '0')) {
      return position[0][i];
    }
  }

  if (position[0][0] === position[1][1] && position[1][1] === position[2][2] && (position[0][0] === 'X' || position[0][0] === '0')) {
    return position[0][0];
  }

  if (position[0][2] === position[1][1] && position[1][1] === position[2][0] && (position[0][2] === 'X' || position[0][2] === '0')) {
    return position[0][2];
  }
  return undefined;
}
